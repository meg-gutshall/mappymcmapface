// I decided to make a service class in which to build new HTML elements in order to adhere to the Single Responsibility Principle. It also makes the rest of my app look a bit tidier. :)
// By using the `static get` keywords, I can use ES6 syntactic sugar to invoke these class methods in other files where I've imported the DOMElements class.
...
class DOMElements {
  ...
  // This method calls on the `mainBody` method above to access the main content of the page. It then adds a row to hold the current user's topic requests by manipulating the returned element's `innerHTML` property.
  ...
  // This method calls the `createUserRow` method above to execute the creation of the current user topic request row. It then returns the empty row, not the header row that contains the "My Code Talks Topic Ideas" text. However, the header row will still appear in the main content of the site.
  static get userRow() {
    this.createUserRow();
